...
const ENTITY = "google_inbound_items";

function selectGoogleInboundItems(state) {
  return [...(state.data.google_inbound_items || [])].sort((left, right) =>
...
function renderGoogleInbound(copy, items) {
  const statuses = Object.keys(TRANSITION_MAPS[ENTITY]);
  return `
    <article class="app-panel">
      ${renderSectionHeading(copy.modules.google.inbound, copy.modules.google.inbound)}
      <form class="app-create-form" data-create-entity="${ENTITY}">
        <div class="app-form-grid">
          <label><span>${escapeHtml(copy.forms.intentSource)}</span><input name="intent_source" value="form" required /></label>
      ...
      <div class="app-board compact-board">
        ${statuses
          .map((status) => {
                                ...
                                <button class="app-card-main" type="button" data-open-drawer="${ENTITY}:${record.id}">
                                  <div class="app-card-head">
                                    <div>
                                      ...
                                      <p>${escapeHtml(record.intent_source)} • ${escapeHtml(record.contact_name || "—")}</p>
                                    </div>
                                    ${renderBadge(localizeValue(copy, record.status))}
                                  </div>
                                  <p class="app-card-summary">${escapeHtml(record.summary)}</p>
                                  <div class="app-card-meta">${renderBadge(formatShortDate(record.next_step_date, copy.meta.lang), "outline")}</div>
                                </button>
                                ${footer}
                              </article>
                            `;
...
function renderGoogleInboundDrawer(app, record) {
  const { copy } = app;
  return `
    <div class="app-drawer-stack">
      ${renderSectionHeading(copy.modules.google.inbound, record.company_name)}
      <div class="app-detail-grid">
        ${renderKeyValue(copy.forms.intentSource, record.intent_source)}
        ${renderKeyValue(copy.forms.contactName, record.contact_name)}
        ${renderKeyValue(copy.forms.pageOrListing, record.page_or_listing)}
      ...
      <form data-edit-record="${ENTITY}:${record.id}">
        <div class="app-form-grid">
          ...
          <label class="wide"><span>${escapeHtml(copy.forms.reviewSignal)}</span><input name="review_signal" value="${escapeHtml(record.review_signal)}" /></label>
          <label class="wide"><span>${escapeHtml(copy.forms.inboundSummary)}</span><input name="inbound_summary" value="${escapeHtml(record.inbound_summary)}" /></label>
        ...
        record.status === "Qualified" && !record.converted_qualified_lead_id
          ? `
            <form data-convert-source="${ENTITY}:${record.id}">
              <div class="app-form-grid">
                <label class="wide"><span>${escapeHtml(copy.forms.painSummary)}</span><input name="pain_summary" value="${escapeHtml(record.review_signal || record.summary)}" /></label>
...
export { ENTITY as GOOGLE_INBOUND_ENTITY, renderGoogleInbound, renderGoogleInboundDrawer, selectGoogleInboundItems };
